...
function app() {
	...
	// settings
	var grid = 4; // desirable grid size
	var winCond = 64; // desirable number to win
	...
	function shift(direction) {
		for (var x = 0; x < board.length; x++) {
			...
			var i = direction === 'left' || direction === 'top' ? x : board.length - 1 - x;
			var moveStep = false;
			var double = false;
	...
	function addNew() {
		...
		var randomFreeIndex = shuffle(freePlaces).pop();
		var newVal = Math.random() < 0.5 ? 2 : 4;
	...
	function keyHandler(event) {
		...
		if (gaming === true) {
			switch (x) {
				case 38:
					move('top');break; // arrow top
				case 39:
					move('right');break; // arrow right
				case 40:
					move('bottom');break; // arrow bottom
				case 37:
					move('left');break; // arrow left
			}
		}
	}
